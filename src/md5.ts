/* eslint-disable max-len, no-bitwise, no-plusplus */
...
function md5(inputString: string): string {
  ...
  function rh(n: number) { let j; let s = ''; for (j = 0; j <= 3; j++) s += hc.charAt((n >> (j * 8 + 4)) & 0x0F) + hc.charAt((n >> (j * 8)) & 0x0F); return s; }
...
/* eslint-enable max-len, no-bitwise, no-plusplus */
